...
class LiveModeManager:
    ...
    async def connect(self, websocket: WebSocket, session_id: str, language: str = "english"):
        ...
        await self.send_message(session_id, {
            "type": "connected",
            "message": (
                "Live mode activated. I can see and hear you now!" if language == "english"
                else "लाइव मोड सक्रिय। अब मैं आपको देख और सुन सकता हूं!" if language == "hindi"
                else "లైవ్ మోడ్ యాక్టివేట్ అయింది. ఇప్పుడు నేను నిన్ను చూడగలుగుతున్నాను మరియు వినగలుగుతున్నాను!"
            )
        })
    ...
    async def _call_heavy_pest_detection(
        ...
        try:
            ...
            waiting_msg = (
                ...
                if language == "hindi"
                else "🔬 పురుగును విశ్లేషిస్తున్నాము. దయచేసి కొద్దిసేపు వేచి ఉండండి..."
            )
...
"""
    ...
    async def _generate_and_send_tts(
        ...
        try:
            ...
            # Generate TTS audio
            lang_code = "hi" if language == "hindi" else "te" if language == "telugu" else "en"
...
async def websocket_live_endpoint(websocket: WebSocket, session_id: str):
    ...
    - {"type": "init", "language": "english|hindi|telugu"} - Initialize session
